...
function randFloat(min, max) { return Math.round((rand() * (max - min) + min) * 10) / 10; }
...
const scenarios = [
  ...
  {name:'urban-intersection',road:'town',desc:'Complex urban intersection',sMin:20,sMax:50},
  {name:'roundabout',road:'town',desc:'Multi-lane roundabout',sMin:20,sMax:50},
  {name:'parking-garage',road:'custom',desc:'Multi-level parking structure',sMin:5,sMax:30},
  {name:'highway-ramp',road:'motorway_entry',desc:'Highway on/off ramp',sMin:40,sMax:100},
  ...
  {name:'construction-zone',road:'town',desc:'Road with construction zone',sMin:20,sMax:60},
  ...
  {name:'residential',road:'town',desc:'Residential neighborhood streets',sMin:20,sMax:50},
...
const dataSrcs = ['lidar','camera','scanner','aerial','satellite','procedural generation','survey','mobile mapping'];
const measSys = ['3DMS system','Mobile Mapping System','RTK-GPS','aerial scanner','photogrammetry'];
const licenses = ['CC-BY-4.0','EPL-2.0','CC-BY-NC-4.0','MIT','proprietary','CC-BY-SA-4.0','CC0-1.0'];
...
for (let i = 1; i <= 100; i++) {
  ...
  const slug = city.toLowerCase().replace(/ /g,'-')+'-'+sc.name+'-'+String(i).padStart(3,'0');
  const name = city+' '+sc.name.replace(/-/g,' ').replace(/\b([a-z])/g,(m,c)=>c.toUpperCase())+' HD Map';
  const desc = sc.desc+' in '+city+', '+region+'. '+nInt+' intersections, '+nSigns+' signs.';
