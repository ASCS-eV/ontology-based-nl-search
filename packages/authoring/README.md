...
| A10 | Setting `properties.triggerKind` on a `LaneChangeAction` lowers to an entity-based `<ByEntityCondition>` start trigger (`TimeHeadway`/`RelativeDistance`) instead of the default `startTime`-based one; the condition-kind dispatch is generated from the pinned SHACL, never a hand-typed class-name literal in the lowering. | `authoring-wasm/src/__tests__/engine.test.ts` |
