...
/**
 * A realistic cut-in {@link AuthoringIR} — the generic, SHACL-keyed scene the LLM
 * would fill: an ego vehicle plus two neighbours, initial speeds and teleports,
 * and a single lane-change maneuver where `A2` cuts in ahead of the ego. Used to
 * exercise the archetype mapping (`irToEngineTree`) and the end-to-end lowering
 * (`WasmAuthoringBackend.lower`) against the real engine.
 */
