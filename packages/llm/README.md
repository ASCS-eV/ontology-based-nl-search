...
The `agent/` subdirectory (provider adapters, tool schema, Copilot session, submission router) is intentionally **not** exported — consumers go through the facade so internals can be refactored freely. Providers: OpenAI, Anthropic, claude-cli, vibe-cli/Mistral, Ollama (via the Vercel AI SDK), the GitHub Copilot SDK, and the user's own Claude Code binary run headless (`claude-code`, no SDK dependency).
...
| L14 | **Claude Code provider holds the same boundary, in a session with nothing to act with** — only structured output that validates against `slotSubmissionSchema` / `sceneSubmissionSchema` is used, anything else falls back; the headless `claude` runs with `--safe-mode`, no tools, no MCP servers, no transcript, in a fresh empty directory, and no credential of the app reaches it | `src/__tests__/claude-code-cli.test.ts` ("runs the binary isolated…", "withholds the app's credentials…"), `src/agent/__tests__/claude-code-agent.test.ts` ("falls back deterministically when the output does not validate…")    |
