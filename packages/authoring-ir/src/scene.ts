 ...
 * owns the XML. This is the authoring-side security boundary.
 */

/** A scene entity → an OpenSCENARIO `<ScenarioObject>`. */
export interface SceneEntity {
  /** Local id the LLM assigns; must be unique across the scene (SHACL-checked). */
  ref: string
...
export interface SceneAction {
  /** Must resolve to a {@link SceneEntity.ref} (referential integrity). */
  actor: string
   ...
   * Omitted = the action references no other entity.
   */
  references?: Record<string, string>
}

/**
 * The full authoring IR the LLM fills via the `submit_scene` tool —
