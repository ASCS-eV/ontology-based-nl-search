...
| O11 | **Missing ontology sources raise `OntologySourcesError` with actionable, data-driven remediation** — surfaces the `git submodule update --init` hint, else the config-knob hints                                                 | `src/__tests__/sources.test.ts` ("assertOntologySourcesAvailable", "formatMissingSourcesError")                                                                                                                 |
