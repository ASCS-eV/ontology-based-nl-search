...
pipeline and ships as a **git submodule** ([ontology-management-base](https://github.com/ASCS-eV/ontology-management-base)).
Without it the API starts degraded and every search returns empty results.

```bash
# Fresh clone — pull the ontology submodule in one step:
...
> SHACL + JSON-LD context **artifacts** from `ontology-management-base` (the sole
> submodule). The sample simulation-asset data under `packages/search/data/` is
