...
instead pinned in-repo as **machine-readable artifacts** — a submodule commit
where the source is a repository we already vendor, a checksummed manifest where
it is not — and cited inline with the tags below. See
...
Refreshing these means bumping a pinned commit — the submodule pins (see
`packages/authoring-wasm/versions.json`) or a bundle manifest's `commit` field
followed by `node packages/authoring-gate/qc-bundles/refresh.mjs` — not the
proxy download used for the W3C/IETF texts above.
