...
| **Multi-reference + nested chains** | `references` is a list of AND-combined JOINs; entries nest to express `scenario → trace → map`; the compiler emits path-suffix vars and a breadcrumb per reference |
| **Flattened slot shape**            | Country / region / city / license route through `filters` like any other leaf                                                                                      |
| **Traceability — predicate chain**  | `compileSlotsWithTrace` binds intermediate JOIN vars; per-row, per-reference breadcrumb UI                                                                         |
...
| **API authentication**              | Optional `API_KEY` gate (all routes except `/health`); production fails fast unless a key is set or `API_ALLOW_UNAUTHENTICATED=true`                               |
| **CI/CD**                           | GitHub Actions with typecheck + lint + test + E2E; manual `Release` workflow (git-cliff changelog → GitHub Release)                                                |
