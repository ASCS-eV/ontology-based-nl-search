...
| --------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `LLM_TEMPERATURE`     | unset   | Opt-in override; no sampling parameter is sent by default. Anthropic removed `temperature` in the Claude 4.7 generation, so later models — including the default `claude-sonnet-5` — reject it ([`/v1/messages` § Request](https://platform.claude.com/docs/en/api/messages)). Determinism comes from the single-tool submission and the deterministic compiler, not from greedy decoding. Set it only for a provider/model that still accepts sampling parameters. |
| `LLM_THINKING`        | `off`   | Anthropic reasoning mode: `off`, `adaptive`, or a positive token budget. The two on-modes are not interchangeable and the wrong one is a hard 400: `adaptive` needs Claude 4.6+, while a fixed budget was removed in the 4.7 generation and works only on older models. claude-cli/anthropic only; Copilot reasons via `AUTHORING_REASONING_EFFORT`.                                                                                                                |
| `LLM_MAX_AGENT_STEPS` | `3`     | Hard cap on tool-call rounds. With `toolChoice` forcing `submit_slots`, the typical query needs 1 step.                                                                                                                                                                                                                                                                                                                                                             |

Reasoning mode by provider: Mistral uses the `magistral-*` family, OpenAI uses the `o`-series model names (`o1`, `o4-mini`), Anthropic exposes a typed `thinking` block — `LLM_THINKING` is the only var that surfaces it explicitly. Model-id spelling is provider-specific and is passed through verbatim: Copilot uses dots (`claude-haiku-4.5`), the Anthropic Messages API uses hyphens (`claude-haiku-4-5`) and returns a 400 naming the model for the dotted form.
